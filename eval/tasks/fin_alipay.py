...
class FinAlipay(CommonExam):
    def __init__(self,shot,data_path='./exams/fin_alipay',dataset_type='dev'):
        super(FinAlipay, self).__init__(data_path=data_path,shot=shot,categories={})
        self.dataset_type = dataset_type
        assert self.dataset_type in ["dev", "test", "full"],f'加载数据集类型错误，限定在dev、test、full中，当前赋值为：{self.dataset_type}'
        ...
        for dirpath, dirnames, filenames in os.walk(self.data_path):
            for filename in [f for f in filenames if f.endswith(".csv")]:
                ...
                for i in range(len(frame)):
                    ...
                    # test集是未披露answer的评测题
                    # full集是所有评测题
                    if self.dataset_type  == "dev" and answer == "":
                        continue
                    elif self.dataset_type  == "test" and answer != "":
                        continue
    ...
    答："""
                    ...
                    details["subject"] = subject + "+" + domain
    ...
    def __len__(self):
        ...
        for dirpath, dirnames, filenames in os.walk(path):
            for filename in [f for f in filenames if f.endswith(".csv")]:
                path = os.path.join(dirpath, filename)
                frame = pd.read_csv(path)
                answers = frame["answer"].to_list()
                if self.dataset_type  == 'dev':
                    total += len([item for item in answers if not pd.isna(item)])
                elif self.dataset_type  == 'test':
                    total += len([item for item in answers if pd.isna(item)])
                else:
                    raise ValueError("加载数据集类型错误，限定在dev、test、full中，当前赋值为 ：" + str(self.dataset_type))
        return total
